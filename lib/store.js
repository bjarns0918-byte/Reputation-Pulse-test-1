...
// --- Per-business lock ---
//
// readDb()/writeDb() themselves are already safe against true simultaneous
// writes, since they're synchronous and Node only runs one piece of JS at a
// time. The real gap is different: a function that reads a business, then
// `await`s something slow (Claude, Google, Stripe), then writes its changes
// - if a second operation for that SAME business runs during that gap, its
// write can get silently overwritten when the first one finishes.
//
// withBusinessLock() closes that gap: any code wrapped in it waits for any
// other locked operation on the same business to finish first, so a
// scheduled sync and a manual "sync now" click (for example) queue up
// instead of racing.
const businessLocks = new Map();

export async function withBusinessLock(businessId, fn) {
  const previous = businessLocks.get(businessId) || Promise.resolve();
  let releaseLock;
  const thisLock = new Promise((resolve) => { releaseLock = resolve; });
  const chained = previous.then(() => thisLock);
  businessLocks.set(businessId, chained);
  ...
  try {
    ...
    releaseLock();
    if (businessLocks.get(businessId) === chained) {
      businessLocks.delete(businessId);
    }
  }
}
