...
export async function buildDigest(reviews, bizName) {
  ...
  const prompt =
    ...
    `Write a short, professional summary (4-6 sentences) in plain, simple English. Open with one sentence on the overall ` +
    `picture, then note any recurring themes worth knowing (both strengths and issues), naming specific staff members only ` +
    `if they are mentioned by name in the reviews. Flag anything that needs prompt attention. ` +
    `Keep the tone measured and business-appropriate rather than casual - this should read like a concise report, not a ` +
    `chat message. Do not restate every review individually. Do not use bullet points, headers, emojis, or exclamation points. ` +
    `Write it as flowing plain text only.`;
