...
app.get("/api/digest-pdf", requireLogin, (req, res) => {
  ...
  const history = business.ratingHistory || [];
  const latest = history[history.length - 1] || null;
  const label = latest ? latest.label : new Date().toLocaleDateString("en-US", { month: "long", year: "numeric" });
  ...
  // Header
  doc.fillColor("#14213D").fontSize(22).font("Helvetica-Bold").text(business.businessName, { align: "left" });
  doc.fontSize(11).fillColor("#6B7280").font("Helvetica").text(`Monthly digest - ${label}`);
  doc.moveDown(0.3);
  if (latest) {
    doc.fontSize(11).fillColor("#14213D").font("Helvetica-Bold")
      .text(`${latest.avgRating} / 5 average rating   -   ${latest.reviewCount} reviews this period`);
  }
  doc.moveDown(1.2);
  doc.moveTo(56, doc.y).lineTo(539, doc.y).strokeColor("#E2E4DE").stroke();
  doc.moveDown(1);

  // What's working
  doc.fontSize(14).fillColor("#2D9C8A").font("Helvetica-Bold").text("What's working");
  doc.moveDown(0.3);
  doc.fontSize(11).fillColor("#333333").font("Helvetica").text(business.latestDigest.strengths || "Not enough data yet.", {
    align: "left",
    lineGap: 4
  });
  doc.moveDown(1);
  ...
  // Needs improvement
  doc.fontSize(14).fillColor("#D64545").font("Helvetica-Bold").text("Needs improvement");
  doc.moveDown(0.3);
  doc.fontSize(11).fillColor("#333333").font("Helvetica").text(business.latestDigest.improvements || "Not enough data yet.", {
    align: "left",
    lineGap: 4
  });

  doc.moveDown(2);
  doc.fontSize(9).fillColor("#6B7280").text("Generated by RepAnchor - repanchorapp@gmail.com", { align: "center" });
