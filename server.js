...
// Limits how often the "Run preview" demo feature can be used - keeps a
// hesitant/bad-faith signup from generating unlimited free AI content
// before connecting Google. Your admin account is exempt, so testing stays
// unlimited.
const previewLimiter = rateLimit({
  ...
  legacyHeaders: false,
  handler: rateLimitHandler,
  skip: (req) => !!(req.business && req.business.isAdmin)
});
...
app.post("/webhook/stripe", express.raw({ type: "application/json" }), (req, res) => {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    if (businessId) {
      const wasFirstTime = !getBusinessById(businessId)?.stripeCustomerId;
      ...
      if (business && wasFirstTime) {
        const html = `
          <p style="font-size:16px; font-weight:700; color:#14213D; margin:0 0 14px;">Welcome to RepAnchor, ${business.businessName}!</p>
          <p>Your 7-day free trial has started - here's what happens next:</p>
          <ol style="padding-left:20px;">
            <li style="margin-bottom:8px;">Log in and click <strong>"Connect Google"</strong> to link your Google Business Profile.</li>
            <li style="margin-bottom:8px;">Once connected, we'll automatically post replies to your best reviews and draft replies for anything trickier, waiting for your approval.</li>
            <li style="margin-bottom:8px;">Every month, you'll get a simple digest showing what's working and what needs attention.</li>
          </ol>
          <p>Not sure where to start? Just reply to this email or reach out at <a href="mailto:repanchorapp@gmail.com">repanchorapp@gmail.com</a> - we read every message.</p>
          <p style="margin-top:20px;"><a href="${process.env.BASE_URL}/" style="display:inline-block; background:#14213D; color:#fff; padding:10px 18px; border-radius:7px; text-decoration:none; font-size:14px;">Go to your dashboard</a></p>
        `;
        sendEmail(business.email, "Welcome to RepAnchor", html).catch((err) => {
          console.error(`[stripe] Failed to send welcome email to ${business.email}:`, err.message);
        });
      }
    }
...
// Admin-only overview of every real customer account - lets you see who's
// active, trialing, or canceled at a glance without digging through Stripe.
app.get("/api/admin/businesses", requireLogin, (req, res) => {
  ...
  const businesses = getAllBusinesses()
    ...
    .map((b) => ({
      ...
      googleConnected: !!b.googleTokens,
      lastSyncedAt: b.lastSyncedAt || null,
      createdAt: b.createdAt,
      teamMemberCount: (b.teamMembers || []).length
    }))
...
app.post("/api/run-simulation", requireLogin, previewLimiter, requireActive, async (req, res) => {
  try {
    const business = req.business;
...
// Generates a clean, downloadable PDF version of the latest monthly digest.
app.get("/api/digest-pdf", requireLogin, (req, res) => {
  const business = req.business;
  if (!business.latestDigest) {
    return res.status(404).json({ ok: false, error: "No digest available yet - run a preview or wait for your first monthly digest." });
  }

  const history = business.ratingHistory || [];
  const latest = history[history.length - 1] || null;
  const monthLabel = latest ? latest.label : new Date().toLocaleDateString("en-US", { month: "long", year: "numeric" });

  res.setHeader("Content-Type", "application/pdf");
  res.setHeader("Content-Disposition", `attachment; filename="${business.businessName.replace(/[^a-z0-9]/gi, "_")}_digest.pdf"`);
  ...
  doc.rect(0, 0, pageWidth, bandHeight).fill(navy);

  // Small vector anchor mark, drawn directly (crisp at any size, no image file needed)
  doc.save();
  doc.translate(marginX, 34);
  doc.circle(0, 0, 3.2).lineWidth(1.6).stroke(gold);
  doc.moveTo(0, 3.2).lineTo(0, 17).lineWidth(1.9).stroke(gold);
  doc.moveTo(-7, 8).lineTo(7, 8).lineWidth(1.9).stroke(gold);
  doc.moveTo(-12, 13).quadraticCurveTo(-13, 21, -4.5, 23).lineWidth(1.9).stroke(gold);
  doc.moveTo(12, 13).quadraticCurveTo(13, 21, 4.5, 23).lineWidth(1.9).stroke(gold);
  doc.restore();

  doc.fillColor("#ffffff").fontSize(12).font("Helvetica-Bold").text("RepAnchor", marginX + 22, 26);

  doc.fillColor("#ffffff", 0.55).fontSize(9).font("Helvetica")
    .text(monthLabel.toUpperCase(), marginX, 56, { characterSpacing: 0.6 });
  doc.fillColor("#ffffff", 1).fontSize(18).font("Helvetica-Bold").text(business.businessName, marginX, 70);

  if (latest) {
    doc.fillColor(gold, 1).fontSize(26).font("Helvetica-Bold")
      .text(`${latest.avgRating}\u2605`, 0, 34, { align: "right", width: pageWidth - marginX });
    doc.fillColor("#ffffff", 0.6).fontSize(9).font("Helvetica")
      .text(`${latest.reviewCount} reviews this period`, 0, 66, { align: "right", width: pageWidth - marginX });
  }

  // --- Body ---
  let y = bandHeight + 40;

  doc.fillColor(teal).fontSize(14).font("Helvetica-Bold").text("What's working", marginX, y);
  y = doc.y + 8;
  doc.fillColor("#333333").fontSize(11).font("Helvetica")
    .text(business.latestDigest.strengths || "Not enough data yet.", marginX, y, {
      width: pageWidth - marginX * 2,
      lineGap: 4
    });
  y = doc.y + 28;

  doc.fillColor(coral).fontSize(14).font("Helvetica-Bold").text("Needs improvement", marginX, y);
  y = doc.y + 8;
  doc.fillColor("#333333").fontSize(11).font("Helvetica")
    .text(business.latestDigest.improvements || "Not enough data yet.", marginX, y, {
      width: pageWidth - marginX * 2,
      lineGap: 4
    });

  doc.fillColor(muted).fontSize(9).font("Helvetica")
    .text("Generated by RepAnchor - repanchorapp@gmail.com", marginX, doc.page.height - 50, {
      align: "center",
      width: pageWidth - marginX * 2
    });
